...
- Backend tests and package build pass: 32 tests, no failures or skips.
- Frontend tests, typecheck, lint, and production build pass: 16 tests across
  three files.
- Flyway applies the Phase 1 schema to a PostgreSQL Testcontainer.
- The Compose configuration is valid and all three local services are healthy.
- The ordinary Lark-disabled backend starts healthy and reports disabled state.
- The restored Lark-enabled backend is healthy and the WebSocket reports
  connected.
- Aggregate production-like local state records six eligible direct messages,
  six successful replies, and no claimed processing record for Hongye's
  rejected non-pilot direct message.
- Victor completed the live H5 authorization flow inside Lark; the application
  displayed `Running in Lark`, `Welcome, Victor`, and `Authorized`.
- Hongye, an explicitly available non-pilot teammate, completed the same live H5
  entry flow and was refused with the Victor-only pilot message. The rejected
  attempt created no additional user connection and did not replace Victor's
  active encrypted connection.
- The live authorization created one single-use authorization-code claim and one
  active encrypted user connection. Aggregate checks confirmed both token
  ciphertexts are present without printing identities or token values.
- After a backend restart, the encrypted connection remained in PostgreSQL and
  the Lark WebSocket returned to connected state.
- The controlled near-expiry refresh test decrypts the stored refresh token,
  rotates both returned tokens on demand, persists the replacement ciphertexts,
  and marks terminal failures for reauthorization without making an unsafe live
  token-expiry mutation.
- The local `.env` file is ignored and has mode `600`.
- A boolean-only recent-log scan found no token, connection-ticket, or known
  deterministic message-body patterns.

Closure decision:

- The non-pilot direct-message check passed: Hongye received no response while
  Victor received the expected deterministic reply.
- Victor explicitly waived the live group-message check for Phase 1 on
  2026-08-17. Group messages remain unsupported, and the automated SDK policy
  and handler tests verify that they are rejected without a response.

All other automated and live acceptance evidence is complete, so the Phase 1
goal is ready to close.
...
5. Send one deterministic acknowledgement as a normal direct-chat response,
   preserving a reply anchor only when the user explicitly initiated one.
...
| Phase 2 | One evolving streaming answer, H5 typing wave, prompt composer, high-level activity display, tool chips, follow-up prompts |
...
- [ ] Confirm that Bot and Web App/H5 capabilities are enabled in the same Lark
      custom application.
- [ ] Set the published version's default feature to Web App for both PC and
      mobile so Workplace opens H5 while Messenger remains the Bot entry point.
...
- [ ] Send an ordinary direct message as a normal chat response without quoted
      reply decoration.
- [ ] Preserve the reply anchor when Victor explicitly uses Lark's Reply action.
...
- [ ] Send one standalone text direct message from Victor and receive one
      deterministic normal chat response without quoted reply decoration.
- [ ] Use Lark's Reply action and confirm the deterministic response preserves
      the explicit reply anchor.
- [ ] Verify a repeated delivery or replay fixture does not create a second
      application reply.
- [x] Verify a direct message from a non-pilot account receives no Synvo reply.
- [x] Live group-message check explicitly waived for Phase 1; automated policy
      and handler rejection tests pass.
...
2. A direct text message from Victor receives exactly one deterministic Synvo
   acknowledgement using the correct normal-message or explicit-reply routing.
...
- [x] Every acceptance criterion is satisfied, with the live group-message check
      explicitly waived and covered by automated rejection tests.
- [x] All required automated tests and controlled live Lark smoke tests pass.
- [x] No real credentials, IDs, tokens, encryption keys, or tunnel URLs are
      committed.
- [x] Direct-message processing remains deterministic and Victor-only.
- [x] The React client has no access to Lark user tokens.
- [x] PostgreSQL contains no plaintext token or message-content values.
- [x] The codebase remains one React application, one Spring Boot modular
      monolith, and one PostgreSQL database.
- [x] No Phase 2 Agent Core or model work has been pulled into Phase 1.
